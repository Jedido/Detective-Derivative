...
public class Fail1State extends GameState {

	private double textappear;
	private int currentChoice;
	
	private String[] story = {
			"You arrive, but the place is empty.",
			"You must have gone to the wrong area,\n"
			+ "and now he has probably ran away!",
			"",
			...
			"",
			"Hints:",
			"Find the derivative first, then plug in\n"
					+ "the y value",
					"",
					"Function can be integrated in parts", 
					"Bring the exponent \"down\", then subtract by 1", 
			};
	private String[] options = {"Find the derivative first, then plug in\n"
			+ "the y value", 
			"Function can be integrated in parts", 
			"Bring the exponent \"down\", then subtract by 1", 
			};
	...
	public Fail1State(GameStateManager gsm) {
		this.gsm = gsm;
		lineNum = 0;
		init();
	}
